...
| F05 | 首屏处理模式 | 规则可配置“隐藏原内容/仅预编译/关闭”三种策略 | ✅ 必须 |
...
| 策略 | 目的 | 细节 |
| --- | --- | --- |
| **首屏隐藏** | 防止“原文闪现” | 命中规则且模式为“隐藏”时，在 `document_start` 给 `body` 加 `cc-hidden`，完成初次替换后立即移除（300ms 兜底） |
...
4. 首屏隐藏策略不应造成页面长期空白（300ms 兜底必须生效）。
